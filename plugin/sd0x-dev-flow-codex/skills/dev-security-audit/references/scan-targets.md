...
| Path | Contains | Risk |
|------|----------|------|
| ~/.config/gh/hosts.yml | GitHub CLI OAuth token (may ref keychain); if compromised: hand off GitHub session revocation and re-binding to a separate remediation workflow | Critical |
