...
- 具 `codex plugin` 指令的 Codex CLI。本文最後以 Codex CLI `0.144.4` 校準；若版本不同，先檢查第 14 節的版本邊界。
...
實測 Codex CLI `0.144.4` 會忽略 symlink `SKILL.md`。因此 skill entrypoint 不能 live-link；修改任何 `SKILL.md` 後必須重建 overlay 並開新 task。Skill 內 bundled scripts/resources 仍可維持 symlink live update。
...
- Codex `0.144.4` 沒有正式 per-project plugin install；repository-only 模式是隔離 `CODEX_HOME`，主要服務 CLI。
