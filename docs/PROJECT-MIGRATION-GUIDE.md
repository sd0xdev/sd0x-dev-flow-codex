...
4. Auto-loop 必須持續到 fingerprint-bound gates 通過，並提供使用者明確操作的 reset 以處理 stale runtime evidence。
...
- 8 個 skills：`setup`、`review`、`verify`、`feature-dev`、`bug-fix`、`remind`、`reset`、`doctor`。
...
- Review/verify pass 後若又啟動同 fingerprint reviewer，`nextAction` 會在 start ledger 清空前回到 review-in-progress，Stop 不得提前放行；clean terminal result 會恢復既有 pass eligibility。
...
- Auto-loop 沒有固定 round 或 continuation 上限；dirty worktree 只會在目前 fingerprint 的必要 gates 全部通過後正常結束。
- 使用者明確執行 `$sd0x-dev-flow-codex:reset` 時，runtime 會保留 active sessions 與 worktree、rotate runtime epoch、清除 review／verify gates 及 reviewer evidence，並讓 dirty worktree 立即回到 review-required；reset 前啟動而在 reset 後完成的 reviewer evidence 不得寫回，reset 也不能繞過 gate。
...
stateDiagram-v2
  ...
  ReviewRequired --> ReviewRequired: Stop continuation
  VerifyRequired --> VerifyRequired: Stop continuation
...
| `Stop` | 一般缺 review/verify 時持續回傳 `decision: block`；必要 gates 完成時正常放行。唯一的 fail-preserving 例外是 `reason: reviewer-unavailable`：回傳 `continue: true` 與 user-authorized reset guidance，把控制權交回使用者，但 failed gate 與 stale ledger 會保留到 reset 或真正 fingerprint change。 |
...
2. 說明舊 state 是 migrate 還是安全 reset；v1–v5 會保留可辨識的 activated sessions、清除 pre-provider gates／reviewer evidence；其他 version mismatch 會回到 default state。
...
Reset 會保留 active sessions 與目前 worktree snapshot、rotate runtime epoch，只清除目前 Git worktree 的 sd0x review／verify gates 及 reviewer evidence；dirty worktree 會立即重新要求 review。reset 前啟動但較晚完成的 reviewer result 會因 matching start ledger 已清除而被拒絕。它不是用來繞過應完成的 review/verify。
