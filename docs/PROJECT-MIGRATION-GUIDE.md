...
- 7 個 skills：`setup`、`review`、`verify`、`feature-dev`、`bug-fix`、`remind`、`doctor`。
- 1 個 bundled MCP server：`sd0x_claude_review`，透過本機 Claude CLI 提供主要外部 review 視角。
- 2 個 project-scoped reviewers：`sd0x_reviewer`、`sd0x_test_reviewer`。
...
- Review pass 必須實際觀察到同 fingerprint 的 Claude MCP `PostToolUse` structured clean result，以及兩種 Codex reviewer 的 `SubagentStart` 和 terminal `last_assistant_message` 精確回報 `No actionable findings remain.` 的相符 `SubagentStop`；review evidence 同時必須是 `reviewers: 3`、三種必要 reviewer identity 與 `findings: 0`。
- Claude MCP 的輸入 fingerprint、structured output fingerprint 與 hook 當下 snapshot 必須完全相同；失敗、無 structured output 或 stale result 都不記 evidence。
- 任一 reviewer 對目前 fingerprint 記過 findings 後會保持 blocking；後續同 fingerprint 的 clean result 不得覆蓋。只有 worktree edit 產生新 fingerprint 才能清除。
- Reviewer 在舊 fingerprint 啟動、期間 worktree 被修改，其 Stop 不可計入新 fingerprint。
- Runtime 同時追蹤多個 activated session；新 session 從自己的 continuation count 0 開始，既有 session 的 resume／compact 不會重設限額或停用其他 session。
...
- Worktree 回到 clean 時重設 round state，之後的新任務取得完整 retry budget。
- 預設 `max_rounds = 8`、`max_continuations = 8`，可由 project config 調整，範圍限制為 1 到 100。
...
stateDiagram-v2
  ...
  ReviewRequired --> VerifyRequired: Claude + both Codex reviewers clean
  ...
  ReviewRequired --> Escalate: retry limit reached
  VerifyRequired --> Escalate: retry limit reached
```
...
| `PreToolUse` | 在 session activation 判斷前解析 Codex `apply_patch` command，讓 inactive/setup-deferred session 也一律阻擋 protected paths。 |
| `PostToolUse` | Edit tool 會 refresh fingerprint；Claude MCP tool 只在成功 structured result 與目前 fingerprint 相符時記 external review evidence。 |
...
| `Stop` | 缺 review/verify 時回傳 `decision: block`；完成時放行；超限時 escalation。 |
...
- `mcp__sd0x_claude_review__review_worktree`：blocking primary；同時檢查 implementation 與 test/AC，透過 `safe-mode` Claude CLI 執行。
...
相較來源 `sd0x-dev-flow`，這裡刻意採更嚴格的收斂規則：不收 Nit，任何 P0/P1/P2 都阻擋；Claude primary 與兩個 Codex perspectives 全部是 blocking evidence，不允許 degraded pass；任何 edit 都以新 fingerprint 做 fresh full scan。為保留 root-cause revalidation，fix round 可把同一 reviewer 自己上一輪的 finding identities 當作非權威 hypotheses 傳回，但不可跨 reviewer 分享，也不能當 gate evidence。

Claude adapter 不開放 Bash、Edit、Web 或 project customizations；它接收由 built-in Git 命令建立的 bounded diff/untracked bundle，並只開放 Read/Glob/Grep 研究 surrounding code。預設先給 `claude-fable-5` 15 分鐘與 12 agentic turns；該次完整嘗試若失敗，才以 `claude-opus-4-8` 重跑一次，第二次使用相同限制。Turn cap 透過 Claude 官方 `CLAUDE_CODE_MAX_TURNS` 環境變數設定，避免某些版本支援但未在頂層 `--help` 顯示 `--max-turns` 所造成的 preflight false negative。MCP client 取消或 transport 關閉會終止 active child，不會啟動 fallback；外層 MCP timeout 為 31 分鐘，以容納兩次嘗試及清理時間。可用 `SD0X_CLAUDE_REVIEW_MODEL`、`SD0X_CLAUDE_REVIEW_FALLBACK_MODEL`、`SD0X_CLAUDE_REVIEW_TIMEOUT_MS` 與 `SD0X_CLAUDE_REVIEW_MAX_TURNS` 覆寫；timeout override 只能降低，超過 15 分鐘會 clamp。Protected changed paths、tracked binary changes、nested repository/submodule changes、oversized或無法完整納入的 changed content、Claude CLI/auth failure、兩個 model attempts 都失敗、invalid structured output 與 review 期間 fingerprint 改變全部 fail closed。
...
若有 finding 或任一 reviewer unavailable：記錄 fail、修正或恢復 reviewer、重新取得 fingerprint，並重新啟動三個 perspectives。不可在修正後沿用修正前的 reviewer output。
...
| Agent template | 否 | 否 | 是 | 否 | 是 |
| `.codex/sd0x-dev-flow.json` | 下一次 hook read | 否 | 啟用邊界需要 | 否 | 否 |
...
1. 評估是否提升 `SCHEMA_VERSION`；目前 runtime state schema 是 v4。
2. 說明舊 state 是 migrate 還是安全 reset；v1/v2/v3 會保留 activated sessions、清除舊 gates／reviewer evidence／round，其他 version mismatch 會回到 default state。
...
- `state.test.js`：Claude + dual-Codex clean outcomes、schema migration、fingerprint binding、session retention、round／continuation semantics。
...
npm run check
npm run dev:local:unlink
CODEX_HOME="$PWD/.codex-dev-home" codex plugin add sd0x-dev-flow-codex@sd0x-dev-flow-local --json
CODEX_HOME="$PWD/.codex-dev-home" codex plugin list
npm run dev:local:link
CODEX_HOME="$PWD/.codex-dev-home" codex plugin list
```
...
1. 確認 `/hooks` 已 trust。
2. 確認 bundled `sd0x_claude_review` MCP server 已啟動且本機 `claude` CLI 已登入；呼叫 `$sd0x-dev-flow-codex:setup`，確認產生 `.codex/sd0x-dev-flow.json`、兩個 `.codex/agents/*.toml` 與 `AGENTS.md` managed block。
...
5. 讓 Claude MCP primary 與兩個 Codex reviewers 完成後記錄 review pass，再執行 verify；確認三者 fingerprint 相同且 final Stop 放行。
6. 修改 worktree，確認舊 review/verify evidence 失效。
...
- Claude MCP + dual Codex reviewers → review pass → verify pass → final Stop 放行的完整 E2E。
...
先檢查 Claude MCP 是否成功回傳 structured result、PostToolUse hook 是否已 trust 並記錄同一 fingerprint，再檢查兩種 agent type 的 start/stop 期間 worktree 是否未改變。直接填 `reviewers: 3` 不足以通過 runtime gate。
...
MCP failure 不會降級成 pass。修復環境後必須重跑完整三方 review。
...
使用 doctor 或 status 查看實際 Git metadata state path。不要手動修改 state 來繞過 gate；確認沒有需要保留的 evidence 後，可在目標 repository root 執行：

```bash
node "$HOME/Project/sd0x-dev-flow-codex/plugin/sd0x-dev-flow-codex/scripts/runtime/cli.js" reset
```

若 plugin repo 不在預設位置，將路徑換成實際 checkout。Reset 只清除目前 Git worktree 的 sd0x runtime state，下一次 snapshot 會重建狀態；它不是用來繞過應完成的 review/verify。
...
- Claude MCP primary 依賴本機 Claude CLI 與其有效登入；未安裝、未登入或 managed policy 禁止時 review gate 會 fail closed。
...
對稱採用來源專案的跨模型 review 邊界：Codex host 透過 bundled local MCP adapter 呼叫 Claude primary reviewer，同時保留 implementation 與 test 兩個 native Codex subagents。Claude primary 內部可由 Fable 5 在失敗後重試 Opus 4.8；三個 reviewer perspectives 仍都必須對同 fingerprint clean，不允許缺少任一 perspective 的 degraded pass。
...
- [ ] `npm run check` 全通過。
- [ ] Claude MCP primary 與兩個 Codex reviewers 對目前 fingerprint 無 actionable findings。
