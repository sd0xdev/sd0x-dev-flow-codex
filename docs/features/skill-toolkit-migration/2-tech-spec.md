...
```json
{
  ...
  "alias_policy_decision": {"policy":"mapping-only","codex_version":"codex-cli 0.145.0","evidence":"migration/alias-capability.json","rationale":"version-bound registry capability result"},
  "canonical_targets": {
    "doctor": { "modes": ["claude"] },
...
Evidence commit tree 固定存 `records/<kind>/<record_sha256>.json`。Promotion 使用 `evidence/<record>/review.json`、`verify.json`；closure pending 使用 `subject-review.json`、`verify.json`、`ac.json`、`checks.json`；closure final 使用獨立 `docs-review.json`。Record 內各 SHA 對對應 raw canonical redacted blob bytes計算。Current review blob只保留 configured primary 的 Codex terminal result，Claude provider則保留該 primary 的 structured result；舊 schema 的雙 reviewer results只供歷史稽核，不能取得 current gate authority。verify/check blob 保留 runner、argv、exit/duration與 bounded redacted stdout/stderr；AC blob 保留 ordered per-AC verdict/evidence。Evidence record schema v2 要求每個 Complete AC 至少一個 request 外部位置；schema v1 只為稽核與明確 supersession 相容，不能新增、fresh apply或 finalize，但既有 legacy apply journal仍可 recover。Versioned redactor 移除 secrets/account/absolute-path data，無法安全 redaction 就拒絕 ledger write。Evidence commit 以 prior ref commit 為 parent；re-audit 必須從 ref tree 讀 blobs、重算 hashes/chain，不能信任孤立 digest。合法內容變更要 fresh gates + linked revision；missing/tampered blob、stale/mismatch 都 fail。Retirement record 的 payload/verify 可為 `null`，但要有 reason、request/AC hashes、review blob與 approved disposition。

Fresh clone 必須顯式取得 evidence ref 與全部 subject anchors，例如 `git fetch <remote> refs/sd0x-dev-flow-codex/evidence/v1:refs/sd0x-dev-flow-codex/evidence/v1 'refs/sd0x-dev-flow-codex/subjects/*:refs/sd0x-dev-flow-codex/subjects/*'`；離線搬移的 user-approved Git bundle 也必須包含兩組 refs。只取得 evidence ref、只保留 feature branch 或只 checkout full `main` history都不符合 portability contract。Publication 是 external mutation，必須 preview exact remote/ref/**approved immutable local commit OID**/expected old OID 並另行 approval。首次 evidence create 使用 remote compare-and-swap `git push --force-with-lease=refs/sd0x-dev-flow-codex/evidence/v1: <remote> <approved-local-oid>:refs/sd0x-dev-flow-codex/evidence/v1`；empty expected OID 是 actual mutation guard，且 refspec 不讀可變 local ref。後續先驗 approved local OID descendant of fetched remote OID，再用 `--force-with-lease=<ref>:<exact-old-oid> <remote> <approved-local-oid>:<ref>` 做 single-ref CAS。每個 subject anchor 只允許首次 create exact `head_sha`，已存在時必須解析為相同 OID；publication preview/push 要逐一列出 exact ref/OID，不可用可變 branch 代替。Local refs 在 approval 後前進不會改 command source；若 policy要求發布 newest OID，必須重新 preview/approval。禁止 unleased force、auto-merge或 retry with new expectation。已知曾發布卻 missing、divergence、metadata loss或 unavailable import 都 fail closed。Local append 是 promotion workflow 內的 Git-metadata transition；fetch/import/push 不可從最初 migration request 推論授權。
...
**R3 implementation record (2026-07-12)**：runtime state 現以 append-only single-parent evidence commits保存 closed per-kind records與 versioned canonical redacted blobs；每次 append與 replay先完整 re-audit舊 ref，再以 state lock + exact old OID CAS 前進。Commit subject anchor 與 evidence ref 以單一 ref transaction 原子更新，fresh clone、CI、release 與離線 bundle 都搬移 evidence + subject refs，因此 squash merge/branch deletion 不會移除 closure history。每個 evidence commit與 final tree的 paths 必須精確等於該 record kind 的 record + required blobs；writer/auditor拒絕修改/刪除歷史、extra/orphan/misplaced paths、stale redactor、blob/record hash drift、跨 kind/unit或未連續 supersedes chain。Dirty closure subject 綁 exact fingerprint + HEAD；clean commit subject先以 runtime `commit-review-begin` 清空舊 gate/ledger並開啟 `fingerprint=clean` 的 Codex-only collaboration round，SubagentStart context強制 exact base..head/tree，persistent dispatch與 configured primary 的 terminal result必須帶 canonical subject hash，且 attestation只把該 primary reviewer start binding持久化進 current review evidence；只有同 runtime epoch/provider、begin後一個 subject-bound configured primary terminal result與 deterministic verify可由 `commit-review-attest`交給 prepare。schema v8雙 reviewer evidence與更舊三視角 evidence只可歷史稽核，不能取得 current authority；caller-authored hashes、reset/provider drift同樣無效，Claude provider在等價 range adapter落地前 fail closed。所有 represented aggregate/command exits必須一致為 zero且至少有一個具名 invocation；docs-only exemption由 pending record的 closed `verify_required=false`保存，且只接受 exact `{not_required:true}`。逐 AC 必須 `Complete/High` 且有 file:line evidence，且至少一個位置在 request 外；checkbox、Progress 或其他 request-local 行的任意組合都不能閉環自證。`test/r3-acceptance.test.js` 另將八條 AC 各自綁到 focused runtime symbol、具名 regression 與行為 signal，避免用 generic dispatcher或規格敘述代替可執行 traceability。Prepare與 finalize replay皆回傳相同 record/ref，並重驗 blobs、subject、request bytes、AC definition與 non-request projection；final docs-review envelope綁新 fingerprint且與 subject review獨立。Promotion/pack/retirement從目前 disposition與實際 payload tree重算 evidence，closure request必須等於 unit唯一 `promotion_request`，review/verify blobs各自綁 final fingerprint/provider；`audit-source` 對 delivered unit重新導出 owner path、disposition與payload hashes。Fresh clone explicit ref fetch與offline Git bundle fixtures現搬移完整 prepare→finalize→promotion unit並重驗 request/AC/payload/disposition/review/verify；另涵蓋合法 payload revision、tamper、metadata loss、divergence、retirement與 clean/dirty closure。Bundled `create-request` 明確編排 commit attestation（需要時）與 `closure prepare` → exact write → docs review → `closure finalize`。
...
Canonical identity：web `source_id` 先完成 redirect resolution，再移除 fragment/tracking params、保留其餘 query order並正規化 URL。Production adapter 將 exact canonical HTTPS URL 綁到 exact origin，產生 publisher-level `identity_binding_hash` 與 independence key；它不從 hostname suffix 或 URL path 猜組織/作者，同 origin 的多個 artifacts 因而只算一個 independent publisher。需要 author/issuing-organization 層級時，independent validator 可建立以 exact canonical `source_id` 為 key 的 trusted registry，record 固定為 `{publisher_id, author_id, authority_id, identity_binding_hash, signature}`。Signature 必須由 validator authority 的 Ed25519 private key 產生，scoring helper 只接受 payload 內 pinned `sd0x-host-identity-v1` public key 的驗證結果，API 不接受 caller trust-root argument；matching signed record 優先於 origin fallback。Repository 是 `<canonical-repo-url>@<commit>:<path>#<locator>`，independence key 是 repo URL + commit，publisher/author/binding fields 為 null；community/case 無 signed author record 時只取得 platform-origin independence，URL path segment 永不視為作者。非 HTTPS、redirect 未解析、untrusted authority、invalid signature、source relabel、binding mismatch或無法確定 identity時 fail closed，標記 unresolved而不增加 cross-verification。
...
Before a wave starts, `/create-request` generates single-task delivery tickets from this exact list。Dependency input固定為同一個 `{title, path}` value，禁止把一個 request 的 label配到另一個 path。2026-07-28 已 closure 的 direct-promotion tickets 中，舊 generator 曾把 `R4 — Alias Registry Capability` label配到正確的 Formal Plugin Delivery Model path；這些 exact bytes 是 immutable evidence history，不直接重寫，後續 replacement/re-promotion ticket一律使用修正後的 title/path pair。`create-request` 最初依 §3.5 的 user-authorized `live-bootstrap` 建立與維護 tickets，R3 closure API 上線前最高只能寫 `Candidate Complete`；其 Wave 1 formal promotion 已完成，後續 payload revision 仍必須走相同 re-promotion lifecycle。每票只擁有一個 `promotion_unit_id`，有 ≤ 8 AC、one concern layer、≤ 3-day estimate；overlay's `promotion_request` 指向唯一 current gate owner。所有 tickets 都 close promotion；legacy pack-ready ticket 是 immutable predecessor，不是 current completion。Supporting tickets 不直接改 delivery state；wave umbrella tickets prohibited。
...
| [Deep-research origin identity re-promotion](./requests/2026-07-31-deep-research-origin-identity-repromotion.md) | Add a production HTTPS-origin identity path and superseding payload evidence | Deep-research formal promotion + formal delivery model |
...
Foundation R1–R4 完成後，最初由受限 live-bootstrap `/create-request` 為 Wave 1 建立 child requests；正式 promotion 後的各 wave 仍採相同 just-in-time split，且所有 tickets 都只能由 R3 closure API 寫成 `Completed`。Request creation algorithm：
...
|---|---|---|
| Compatibility aliases | Codex `0.145.0` registry 無 manual-only exclusion mechanism；Foundation R4 已永久選定 `mapping-only`，不建立 live aliases，prompt sampling 不得升級 | Codex version/plugin registry 變更後另開 capability request並重生 version-bound evidence |
