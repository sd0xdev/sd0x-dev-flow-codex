...
Current Codex skills are `bug-fix`、`create-request`、`doctor`、`feature-dev`、`remind`、`reset`、`review`、`setup`、`verify`。其中 review 已把來源的雙 reviewer/auto-loop 理論改成 configured Codex-first/Claude-wrapper primary + 兩個 native Codex perspectives，並綁定 exact worktree fingerprint 與 provider policy。`create-request` 已先提供 Codex-native live workflow 與 query-only resolver；R1–R4 仍為 Pending，因此它尚未取得本規格定義的 durable Wave 1 promotion/closure ledger evidence。
