# Wave 6 Setup Hooks Core Promotion
...
`install-hooks` source behavior is assigned to the canonical `setup/hooks` owner. This ticket owns that exact promotion unit and its Codex-native delivery evidence.
...
- Keep `setup/hooks` as the only positive owner for its exact prompt contract.
- Keep `install-hooks` mapping-only without discovered compatibility entrypoints.

## Scope

| Scope | Description |
|---|---|
| In | Audit and promote the `setup/hooks` payload, routing contract, and durable completion evidence. |
...
|---|---|---|
| `migration/staging/install-hooks/` | Read | Canonical source evidence |
...
| `test/setup-hooks-routing.test.js` | New | Trusted routing contract |
...
| Testing | Complete | Routing, semantic, and static checks passed. Preflight `5b6b111c29328e0f93983b1e2dce53a5c4d8ac466e5ae672520c5c1cf4f23f0a`. Final audit `98ecfc2d3137261c31158c2afc8dcbf4e43cf52e9c16c59bf8137058bc49521d` passed. |
